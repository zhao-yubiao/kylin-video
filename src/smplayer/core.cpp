...
int getRotateAngle(QString file)
{
    ...
    avformat_open_input(&pFormatContext, file.toStdString().c_str(), nullptr, nullptr);
    if(pFormatContext == nullptr)
        return 0;
    ...
    int   m_Rotate = -1;
    tag = av_dict_get(pFormatContext->streams[videoStream]->metadata, "rotate", tag, 0);
    if (tag == NULL)
    {
        m_Rotate = 0;
    }
...
bool isMpeg2VideoCodec(QString file)
{
    avcodec_register_all();
    avformat_network_init();
    int videoStream = -1;
    ...
    for(int i=0; i<pFormatContext->nb_streams; i++)
    {
        if(pFormatContext->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            if (pFormatContext->streams[i]->codec->codec_id == AV_CODEC_ID_MPEG2VIDEO)
            videoStream = i;
        }
    }
...
//#endif
    ...
    m_gnomeSessionManager = new QDBusInterface("org.gnome.SessionManager" , "/org/gnome/SessionManager" ,
...
//当播放音乐时，不是闲置状态，不应锁屏
void Core::playAudioNotInS2idle()
{
    //qDebug() << "ZDEBUG " << __FUNCTION__ << __LINE__ << __FILE__ ;

    QDBusMessage response;

    if (m_gnomeSessionManager->isValid()) {
        response = m_gnomeSessionManager->call("Inhibit", "kylin-video", (quint32)0, "open-kylin-video" ,(quint32)8);
    }

    m_reArg = response.arguments().takeFirst().toUInt();

    if (response.type() == QDBusMessage::ErrorMessage) {
        qDebug() << "ZDEBUG " << "Shutdown::shutdown: error:" << response.errorName() << ":" << response.errorMessage();
    } else {
        qDebug() << "ZDEBUG " << "Shutdown::shutdown: success: response == " << response;
    }
//    m_gnomeSessionManager->call("Uninhibit", reArg);
}

void Core::finishAudioInS2idle()
{
    qDebug() << "ZDEBUG " << __FUNCTION__ << __LINE__ << __FILE__ ;
    m_gnomeSessionManager->call("Uninhibit", m_reArg);
    m_reArg = 0;
}
...
//#endif
    ...
    // ffmpeg 获取视频角度
    qDebug() << file;
    int rotate = getRotateAngle(file);
    qDebug() << "旋转角度 " << rotate;
    // lc add 20201112-1 end
...
//    if(mset.original_video_codec.contains("mpeg2video",Qt::CaseSensitive)){
    if(isMpeg2VideoCodec(file)){

        qDebug() << "ZDEBUG mset.original_video_codec == "<< mset.original_video_codec << __FILE__ <<  __FUNCTION__ << __LINE__;
        proc->setOption("hwdec-codecs", "mpeg2video");
    }
...
//#ifndef Q_OS_WIN
	if (proc->isMPlayer()) {
        if ((pref->vdpau.disable_video_filters) && (pref->vo.startsWith("vdpau"))) {
            ...
            // goto end_video_filters; // lc change 20200924
        }
	} else {
		// MPV
...
//#endif
	...
	// Rotate
    // lc change 20201112-1 先不保存上次旋转角度
...
//	}
    if(!proc->isMPV())
    {
        if (rotate != 0)
        {
            ...
            if(rotate == 90)
                proc->addVF("rotate", QString::number(1));
            else if(rotate == 270)
                proc->addVF("rotate", QString::number(6));
    //        if(rotate == 90)
    //            proc->addVF("rotate", QString::number(0));
    //        if(rotate == 90)
    //            proc->addVF("rotate", QString::number(0));
        }
    }
    // lc change 20201112-1 end
...
//        seek_cmd(secs, 0);
        seek_cmd(mset.current_sec + secs, 2); // lc change 20201009
	}
}
...
//    proc->seek(secs, mode, false);//kobe:Enable precise_seeking (only available with mplayer2)
    // lc change 20201009
    proc->seek(secs, mode, true);//kobe:Enable precise_seeking (only available with mplayer2)
}
