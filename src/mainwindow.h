...
#include <QGSettings>
#endif
#include <QDBusInterface>//wgx add
...
public:
    ...
    bool isTopMostOnSingleDisplay(QWidget *p);

    //wgx add for hide/show sogou
    void show_or_hidesogouime();//hide
    int getinputmode();//获得键盘模式，物理键盘or软键盘
    int iskeyboardVisible();//软键盘 show or hide
    QDBusInterface *sougouinterface;
...
//    void showShortcuts();
    ...
    void Previous() const;
    //zyb add end
    void bottomPaintingTimeout();
    void bottomPaintRunning(QWidget*/*, bool b*/);
    //平板模式适配引入显示问题，暂时屏蔽
    //void onTabletModeHideTitleBtn(QString s);
...
private:
    ...
    bool m_isPlaylistShow; // lc add 20201015
    bool m_isMaximized;
...
//    ControllerWorker *m_controllerWorker = nullptr;
    ...
    //平板模式适配引入显示问题，暂时屏蔽
    //QGSettings* m_tabletListen = nullptr;
