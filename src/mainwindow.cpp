...
MainWindow::MainWindow(QString arch_type, QString snap, /*ControllerWorker *controller, */QWidget *parent, Qt::WindowFlags flags)
	...
	, was_minimized(false)
#endif
    , m_isPlaylistShow(false) // lc add 20201015
...
//    , m_controllerWorker(controller)
    ...
    , m_escWidget (0)
    //, m_isTablet (false)     //平板模式适配引入显示问题，暂时屏蔽
{
...
//    this->setGraphicsEffect(effect);
    ...
    //wgx add sogou dbus
    // sogou
    QString str = "com.basesogouimebs_service.hotel_";
    ...
    qDebug()<<str;
    sougouinterface = new QDBusInterface(str, "/",
...
#if 0
    if (m_tabletListen) {
        delete m_tabletListen;
        m_tabletListen = nullptr;
    }
#endif
    //wgx add
    if (sougouinterface) {
        delete sougouinterface;
        sougouinterface = nullptr;
    }

    if(m_core->m_gnomeSessionManager->isValid()){
        m_core->m_gnomeSessionManager->call("Uninhibit",m_core->m_reArg);
    }
}
...
void MainWindow::setBypassCompositor(WId wId, long comp)
{
...
//cpu core offline/online  压力测试不过，功能废弃
#if 0
    switch (compositor) {
    case 1:{
        QDBusMessage offResponse;

        QDBusInterface offLineDusInterface("org.ukui.cpu.conf", "/org/ukui/cpu/core", "org.ukui.cpu.conf",
            QDBusConnection::systemBus());

        offResponse = offLineDusInterface.call("offline_cpu_core");

        if (offResponse.type() == QDBusMessage::ErrorMessage) {
            //qDebug() << "ZDEBUG " << "Shutdown::shutdown: error:" << offResponse.errorName() << ":" << offResponse.errorMessage();
        } else {
            int offValue = offResponse.arguments().takeFirst().toInt();
        }
    ...
    case 2:{
        QDBusMessage inResponse;

         QDBusInterface inLineDusInterface("org.ukui.cpu.conf", "/org/ukui/cpu/core", "org.ukui.cpu.conf",
             QDBusConnection::systemBus());

         inResponse = inLineDusInterface.call("inline_cpu_core");

         if (inResponse.type() == QDBusMessage::ErrorMessage) {
             //qDebug() << "ZDEBUG " << "Shutdown::shutdown: error:" << inResponse.errorName() << ":" << inResponse.errorMessage();
         } else {
             int inValue = inResponse.arguments().takeFirst().toInt();
         }
        break;
    }
    default:
...
bool MainWindow::isTopMostOnSingleDisplay(QWidget* p)
{
    Display *display;
    Window   root_window;
...
#if 0  //TODO: test if videowindow is in topmost
        root_window = DefaultRootWindow(display);
        printf("root_window = %d\n", root_window);
    Window root_return, parent_return;
       ...
       for(unsigned int i = 0; i < child_num; ++i) {
            ...
            printf("# i: %d: wid: %d, (%d, %d) - ", i, window, attrs.width, attrs.height);
        }

        XFree(child_list);
#else   //TODO: hide sogou IME
#endif
...
void MainWindow::bottomPaintingTimeout()
{
    //double test
        isTopMostOnSingleDisplay(this);

    m_bottomToolbar->m_paintingTimer->stop();
//    m_mplayerWindow->updateVideoWindow(true);
    if (this->m_bottomToolbar->m_isCompositor && m_bottomToolbar->isHidden() && isFullScreen() && !m_isPlaylistShow && isActiveWindow() )
    {
       setBypassCompositor(m_mplayerWindow->window()->winId(), 1);//disableCompositor to save power
       this->m_bottomToolbar->m_isCompositor = false;
    }


}
void MainWindow::bottomPaintRunning(QWidget *p)
{
    if (!this->m_bottomToolbar->m_isCompositor)
    {
        setBypassCompositor(m_mplayerWindow->window()->winId(), 2);
        this->m_bottomToolbar->m_isCompositor = true;
    }
...
void MainWindow::createPlaylist()
{
    ...
    connect(m_playlistWidget, &Playlist::sig_hide, [this]() {m_bottomController->temporaryShow();});
...
//平板模式适配引入显示问题，暂时屏蔽
#if 0
void MainWindow::createTabletListen()
{
    m_tabletListen = new QGSettings("org.ukui.SettingsDaemon.plugins.tablet-mode");
    onTabletModeHideTitleBtn("tabletmode");//判断是否为平板模式，并执行相应操作
    connect(m_tabletListen, SIGNAL(changed(QString)),this,SLOT(onTabletModeHideTitleBtn(QString)));
}

void MainWindow::onTabletModeHideTitleBtn(QString s)
{
    QVariant var = m_tabletListen->get("tablet-mode");
    m_isTablet = var.toBool();
    if(m_isTablet)
    {
        showFullScreen();
        m_topToolbar->menu_button->hide();
        m_topToolbar->min_button->hide();
        m_topToolbar->max_button->hide();
        m_topToolbar->close_button->hide();

        m_bottomToolbar->m_btnFullScreen->hide();
    }
    else
    {
        m_topToolbar->menu_button->show();
        m_topToolbar->min_button->show();
        m_topToolbar->max_button->show();
        m_topToolbar->close_button->show();

        m_bottomToolbar->m_btnFullScreen->show();
//        this->onFullScreen();
    }
}
#endif
...
void MainWindow::createBottomToolBar()
{
    ...
    connect(m_bottomToolbar->m_paintingTimer, SIGNAL(timeout()), this, SLOT(bottomPaintingTimeout()));
    connect(m_bottomToolbar, SIGNAL(paintRunning(QWidget*/*, bool*/)), this, SLOT(bottomPaintRunning(QWidget*/*, bool*/)));
...
//                m_bottomToolbar->updatePlaylistBtnQssProperty(false);
//            }
            // lc end
            if(!m_playlistWidget->enter_or_leave_flag)
            {
                m_playlistWidget->hide();
            ...
            //double check if bottomwidge is in topmost in case of IME is there
            //
            if(isTopMostOnSingleDisplay(this)) {
                printf(" ZDEBUG  %s, %d, start paintingTimer\n", __func__, __LINE__);
                m_bottomToolbar->m_paintingTimer->start();
            }
        }
    });
}
...
//    speed_menu->menuAction()->setIcon(QPixmap(":/res/speed.png"));
    ...
    playPrevAct_Ex = new MyAction(Qt::Key_F8, this, "play_prev");
    ...
    playNextAct_Ex = new MyAction(Qt::Key_F10, this, "play_next");
...
//平板模式适配引入显示问题，暂时屏蔽
    if (event->key() == Qt::Key_Escape/* && !m_isTablet */) {
        if (pref->fullscreen) {
...
void MainWindow::onMinWindow()
{
    ...
    emit this->m_bottomToolbar->paintRunning(this->m_bottomToolbar/*, true*/);
}
...
{
    m_isPlaylistShow = visible; // lc add 20201015
...
void MainWindow::togglePlayAction(Core::State state)
{
    if (state == Core::Playing) {//Stopped = 0, Playing = 1, Paused = 2
        ...
        m_bottomToolbar->onMusicPause();
        // lc change 20201111-2
//        if (m_mplayerWindow) {
        if (!m_mplayerWindow->isShowLogo()) {
        // lc change 20201111-2 end
...
bool MainWindow::event(QEvent * e)
{
    ...
    if ((e->type() == QEvent::ActivationChange) && (!isActiveWindow())) {//Widget 的顶层窗口激活状态发生了变化
        if (!this->m_bottomToolbar->m_isCompositor /*|| b*/)
        {
            setBypassCompositor(m_mplayerWindow->window()->winId(), 2);
            this->m_bottomToolbar->m_isCompositor = true;
        }
    ...
    if ((e->type() == QEvent::ActivationChange) && (isActiveWindow())) {//Widget 的顶层窗口激活状态发生了变化
        ...
        qDebug()<<"Widget 的顶层窗口激活状态发生了变化===影音 顶层";
        show_or_hidesogouime();//wgx add
        m_bottomToolbar->m_paintingTimer->start();
...
void MainWindow::showExitCodeFromMplayer(int exit_code)
{
	if (exit_code != 255 ) {
		...
		d.setWindowTitle(tr("%1 Error").arg(PLAYER_NAME));
        d.setTitleText(tr("%1 Error").arg(PLAYER_NAME));
        d.setText(tr("This format is not currently supported !")
                    /*tr("%1 has finished unexpectedly.").arg(PLAYER_NAME) + " " +
                  tr("Exit code: %1").arg(exit_code)*/);
//        d.setLog(m_mplayerLogMsg);
        d.viewlog_button->hide();
		d.exec();
    }
    this->clearMplayerLog();
}
...
void MainWindow::moveEvent(QMoveEvent *event)
{
    ...
    if(m_resizeFlag)
    {
        switch (m_dir) {
        case LEFTTOP:
            if(geometry().bottomRight() != old_rect.bottomRight())
                setGeometry(old_rect);
            break;
        case LEFTBOTTOM:
            if(geometry().topRight() != old_rect.topRight())
                setGeometry(old_rect);
            break;
        case RIGHTBOTTOM:
            if(geometry().topLeft() != old_rect.topLeft())
                setGeometry(old_rect);
            break;
        case RIGHTTOP:
            if(geometry().bottomLeft() != old_rect.bottomLeft())
                setGeometry(old_rect);
            break;
        default:
            break;
    ...
    if (event->buttons() == Qt::LeftButton) {
		...
		break;
            case LEFTBOTTOM:
                ...
                break;
            case RIGHTBOTTOM:
                t_rect.setBottomRight(event->globalPos());
                setGeometry(t_rect);
                break;
            case RIGHTTOP:
                t_rect.setTopRight(event->globalPos());
                setGeometry(t_rect);
                break;
            default:
                break;
...
//wgx add sogou hide or show
void MainWindow::show_or_hidesogouime()//show_or_hide
{
    qDebug()<<"show_or_hide 键盘";
    int mode=getinputmode();
    int isvisible=iskeyboardVisible();
    if(mode == 0){//物理键盘
        QDBusMessage result = sougouinterface->call("HideStatus");
        qDebug()<<"隐藏 物理键盘";
    } else if(mode == 1){
        if(isvisible == 1)
        {
            QDBusReply<bool> result = sougouinterface->call("ShowKeyboard",false);
            qDebug()<<"隐藏 虚拟键盘result"<<result;
        }
    }
}
int MainWindow::getinputmode()//获得键盘模式，物理键盘or软键盘
{

    QDBusReply<int> reply = sougouinterface->call("GetInputMode");
    qDebug()<<"QDBusReply<int>"<<reply;
    if (reply.isValid()) {
        if(reply == 0)//物理键盘
        {
            //物理键盘 0 or软键盘 1
        ...
        qDebug() << "Call GetInputMode failed";
        return -1;
    }
}
int MainWindow::iskeyboardVisible()//软键盘 show or hide
{
    QDBusReply<bool> reply = sougouinterface->call("IsKeyboardVisible");
    qDebug()<<"QDBusReply<int>"<<reply;
    if (reply.isValid()) {
        if(reply == true)//show
        {
        ...
        qDebug() << "Call IsKeyboardVisible failed";
        return -1;
    }
}
